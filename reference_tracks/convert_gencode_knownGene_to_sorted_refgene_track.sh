...
gencode_knownGene_filename=$1
prefix=$(echo $gencode_knownGene_filename | sed s/.txt// | sed s/.gz//)
echo $prefix

cat ./$gencode_knownGene_filename  | grep -v chrom | awk -F $'\t' 'BEGIN {OFS=FS} { print $2, $4, $5, gensub(/[.][0-9]/, "", "g", $1), "0", $3, $6, $7, "0,0,255",  $8, $9, $10 }' | grep -v _fix | grep -v _random | grep -v _alt | bedtools sort -g ~/p1/ref/GRCh38/hg38.fa.fai |  bgzip > ${prefix}.bed.gz
...
#gsutil -m cp ${gencode_knownGene}.sorted.txt.gz* gs://macarthurlab-rnaseq/reference_tracks/
